...
{ \
    ...
    int local_z = get_local_id(2); \
                                            \
    printf(\"Hello World!\\t My global ID is: (%d, %d, %d)\\tMy group ID is: (%d, %d, %d)\\tMy local ID is: (%d, %d, %d)\\n \", global_x, global_y, global_z, group_x, group_y, group_z, local_x, local_y, local_z); \
} \
"; 
...
int main(int argc, char *argv)
{
    ...
    size_t work_items[3] = {16, 16, 16};
    size_t workgroup[3] = {2, 2, 2};

    err = clEnqueueNDRangeKernel(cmd_queue, kernel, 3, NULL, work_items, 
                                 workgroup, 0, NULL, NULL);
    CHK_ERROR(err);
