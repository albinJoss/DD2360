...
void init(Info *info, ParticleCollection *particleCollection)
{
    ...
    cudaMallocManaged(&particleCollection->particles, info->num_particles * sizeof(Particle));
    // printf("cudaMallocManaged done\n");
    for(int i = 0; i < info->num_particles; ++i)
    {
       particleCollection->particles[i].Velocity = float3{0.7f, 0.6f, 0.5f};
       particleCollection->particles[i].Position = float3{0, 0, 0};
    }
...
void gpu_execution(ParticleCollection *particleCollection, Info *info)
{
    ...
    for(int iteration = 0; iteration < info->num_iterations; ++iteration)
    {
        ...
        timestep_for_gpu<<<grid_size, info->block_size>>>(particleCollection->particles, info->num_particles, iteration);
...
int main(int argc, char *argv[])
{
    ...
    // printf("Main initialiation done\n");
    ...
    // printf("Particle collection free\n");
    cudaFree(particleCollection->particles);
    // printf("Freed\n");
    return 0;
}